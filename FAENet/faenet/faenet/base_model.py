...
class BaseModel(nn.Module):
    ...
    def forward(self, data, mode="train", preproc=True):
        ...
        if mode == "train" or self.regress_forces == "from_energy":
            pass
            # data.pos.requires_grad_(True)    # <change> since we are not using gradients 
        ...
        if self.regress_forces:
            ...
            if mode == "train" or self.regress_forces == "from_energy":
                if "gemnet" in self.__class__.__name__.lower():
                    ...
                    # grad_forces = self.forces_as_energy_grad(data.pos, preds["energy"])   # <change> since no gradient computation
            ...
            if self.regress_forces == "from_energy":
                ...
                if mode == "train":
                    ...
                    # preds["forces_grad_target"] = grad_forces.detach()    # <change> since we're not doing force from grad(energy)
            else:
                raise ValueError(
